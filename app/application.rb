  ...
  def call(env)
    ...
    if req.path.match(/items/)
      ...
      if item =@@items.find{|i| i.name == item_name}
        ...
        resp.status =  #IF a user requests an item that you don't have, then return a 400 and an error message

        resp.write "Item not found"
      end
    else
      resp.status=404  #Your application should only accept the /items/<ITEM NAME> route. Everything else should 404
